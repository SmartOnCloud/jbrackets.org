...
import org.jbrackets.parser.tokens.BaseToken;
import org.jbrackets.parser.tokens.BlockToken;
import org.jbrackets.parser.tokens.ExpToken;
import org.jbrackets.parser.tokens.ExtendsTemplateToken;
import org.jbrackets.parser.tokens.ForLoopToken;
import org.jbrackets.parser.tokens.IfToken;
import org.jbrackets.parser.tokens.IncludeToken;
...
public class TemplateParser implements TemplateParserConstants {
    ...
    // --------- parser ----------
    final public TemplateToken process(String templateName)
    ...
    final public ExtendsTemplateToken extends_tag(String templateName)
	    throws ParseException {
	...
	jj_consume_token(END_BR);
	if (!endblock_Name.isEmpty() && !startblock_Name.equals(endblock_Name)) // check
										// if
										// endblock
										// name
										// is
	...
	{
	    if (true)
		throw new ParseException("expected {% endblock "
			+ startblock_Name + " %}\u005cnfound {% endblock "
			+ endblock_Name + " %}\u005cnat line "
	...
	if (definedBlocks.contains(startblock_Name)) {
		...
		if (true)
		    throw new ParseException(
			    "duplicate block definition {% block "
				    + startblock_Name + " %}");
	    }
	}
	...
	if (name.length() > 0) { // System.out.println("filter:" + name);
	...
	label_3: while (true) {
		...
		if (state == 1)
		    filter.append(token.image);
		if (state == 2)
		    filterParam.append(token.image);
		break;
	    case FILTER:
	...
	jj_la1_0 = new int[] { 0x40, 0x43298, 0x43298, 0x40000, 0x400, 0x70000,
		0x70000, 0x40000, 0x40000, };
    }
